...
export default function GenesisLoading({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    const particleVS = `
      ...
      vec3 snoiseVec3( vec3 x ){
        return vec3( snoise(vec3( x.y - 19.1, x.z + 33.4, x.x + 47.2 )), snoise(vec3( x.z + 74.2, x.x - 124.5, x.y + 99.4 )), snoise(vec3( x.x + 87.2, x.y + 34.6, x.z - 23.4 )) );
      }
      ...
      void main() {
        ...
        vec3 target = vec3(a_target.x, a_target.y, 0.0) * 0.65; 
        ...
        if (u_shockwave > 0.0) {
            ...
            pos.y -= u_shockwave * 1.5; 
        }

        pos.x /= aspect; 
        gl_Position = vec4(pos.xy, 0.0, 1.0);

        // 👑 放大粒子体积，赋予水银质感，爆发时粘稠，聚合时收锐
        gl_PointSize = mix(3.5, 1.5, morphProgress) + a_random.z * 1.5;
    ...
    const particleFS = `
      ...
      void main() {
        ...
        // 模拟液滴法线，Z为球体厚度
        float z = sqrt(1.0 - (d * 2.0) * (d * 2.0));
        float alpha = smoothstep(0.5, 0.1, d) * v_alpha;
        
        // 纯净的亮水银内核
        vec3 coreColor = vec3(0.85, 0.88, 0.92); 
        
        // 基于空间位置和随机相位的欧泊石虹彩散射
        float iriFactor = length(v_pos) * 1.5 + v_random.y * 2.0;
        vec3 edgeColor = cosPalette(iriFactor, vec3(0.6, 0.6, 0.65), vec3(0.4, 0.4, 0.4), vec3(1.0, 1.0, 1.0), vec3(0.3, 0.2, 0.5));
        
        // 物理光照混合：边缘为虹彩，核心为高光亮银
        vec3 color = mix(edgeColor * 1.2, coreColor * 1.5, z);
        
        // 加大透明度，使其看起来像极其稠密的液态金属云
    ...
    const bgFS = `
      ...
      void main() {
        // 👑 严格对齐暗室背景，绝对缝合！
        vec3 bgColor = vec3(0.02, 0.02, 0.02); // #050505 对应的归一化 RGB
        float alpha = 1.0;
    ...
    const render = () => {
      ...
      if (typeof navigator !== 'undefined' && navigator.vibrate) {
         if (timeSec > 1.5 && !sensoryTriggers.current.explode) {
             sensoryTriggers.current.explode = true
             try { navigator.vibrate(15) } catch(e){} // 混沌引爆：极其微弱的一震
         }
         if (timeSec > 4.0 && timeSec < 6.5) {
             const pulseIndex = Math.floor((timeSec - 4.0) / 0.5)
             if (pulseIndex > sensoryTriggers.current.pulse) {
                 sensoryTriggers.current.pulse = pulseIndex
                 try { navigator.vibrate([10]) } catch(e){} // 聚合太极：如同心跳的轻叩
             }
         }
      }
      ...
      if (timeSec > 6.5) {
        ...
        if (!sensoryTriggers.current.shockwave) {
           ...
           if (typeof navigator !== 'undefined' && navigator.vibrate) {
              try { navigator.vibrate([40, 30, 80]) } catch(e){} // 维度撕裂：深邃的重击
           }
           ...
           osc.type = 'sine'; 
           osc.frequency.setValueAtTime(45, actx.currentTime); 
           osc.frequency.exponentialRampToValueAtTime(5, actx.currentTime + 1.2)
  ...
  return (
    <div 
      ...
      style={{ backgroundColor: isShockwave ? 'transparent' : '#050505' }} // 👑 严格对齐暗室背景色
    >
