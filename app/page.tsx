...
import InkFishes from '@/components/ui/InkFishes' 
import GenesisLoading from '@/components/ui/GenesisLoading' 

// 👑 高级钩子：解决 Next.js SSR 闪屏的核心武器
  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      void main() {
        ...
        float twist = -u_vortex * 25.0 * exp(-distToCenter * 2.5); 
...
export default function HomePage() {
  ...
  useIsomorphicLayoutEffect(() => {
    if (!isSpaInitialized) {
      ...
      const isFallbackReload = window.performance.navigation && window.performance.navigation.type === 1

      if (isReload || isFallbackReload) {
        sessionStorage.removeItem('haoye-has-entered')
      }
    }
  ...
  const handleLoadingComplete = () => {
    setHasEntered(true)
    sessionStorage.setItem('haoye-has-entered', 'true')
  }
  ...
  return (
    <>
      ...
      <main className="relative w-full h-[100svh] overflow-hidden bg-transparent">
          ...
          {NAVS.map((n) => (
            <div key={n.id} className={`absolute flex flex-col ${n.pos}`}>
              <FloatingMagnetic floatDelay={n.delay} isActive={state === 'idle'}>
                <motion.div
                  animate={{ opacity: state === 'idle' ? 1 : 0, filter: state === 'idle' ? 'blur(0px)' : 'blur(10px)', scale: state === 'idle' ? 1 : 0.95 }}
                ...
                >
                  <span className={`text-[24px] md:text-[28px] font-light tracking-[0.2em] transition-all duration-700 ${n.textCls}`}>{n.l}</span>
                  <span className={`text-[9px] md:text-[10px] font-mono tracking-[0.4em] mt-3 transition-all duration-700 ${n.subCls}`}>{n.s}</span>
                </motion.div>
              </FloatingMagnetic>
            </div>
          ))}
          ...
          {state === 'dropping' && (
            <motion.div
              ...
              className={`fixed z-50 w-3 h-3 rounded-full pointer-events-none -ml-1.5 -mt-1.5 ${
                dropTheme === 'black' ? 'bg-[#050505] shadow-[0_0_20px_3px_rgba(0,0,0,0.6)]' : 'bg-white shadow-[0_0_20px_3px_rgba(255,255,255,0.9)]'
              }`}
            />
          )}
        </AnimatePresence>
